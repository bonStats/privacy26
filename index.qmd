---
title: "Persuasive Privacy"
subtitle: "ISBA Satellite Meeting, 5^th^ July 2026"
author: "Joshua J Bon"
institute: "Adelaide University*"
bibliography: refs.bib
csl: begel.csl
format:
  revealjs:
    html-math-method: katex
    theme: default
    df-print: paged
    incremental: true 
    css: style.css
    chalkboard: true
    self-contained-math: true
editor: source
---


## Today's talk^[Slides: [bonstats.github.io/privacy26](https://bonstats.github.io/privacy26)]

:::: {.columns}

::: {.column width="50%"}

1. Un petit jeu
2. DP (and MCMC)
3. Persuasive privacy^[Paper: Bon, Bailie, Rousseau & Robert (2026) _ICML 2026_ [arXiv 2601.22945](https://arxiv.org/abs/2601.22945)]
4. DP, I am your father
5. (Private) discussion


:::

::: {.column width="50%"}

![](imgs/leshouches.png)

:::

::::

## A Game of Privacy

:::: {.columns}

::: {.column width="40%"}

::: {.callout-tip}

## What is your prior probability?
_I've met a sumo wrestler._

:::

![](imgs/palace.jpg){.fragment width="500"}

:::

::: {.column width="60%"}

::: {.r-stack}
![](imgs/niseko.JPG){.fragment .fade-in width="450"}

![](imgs/shibuya.JPG){.fragment  width="550"}

![](imgs/sumo-morning.JPG){.fragment  width="450"}

![](imgs/sumo-night.JPG){.fragment width="550"}

![](imgs/sumo-josh.JPG){.fragment width="450"}

:::

:::

::::

## Why "persuasive"?

::: {.fragment}
_Sender chooses a mechanism that **persuades** Receiver to make decisions limiting privacy loss._
:::

::: {.fragment}
Related to **Bayesian persuasion**^[@kamenica2011bayesian] but...
:::

- asymmetry between Sender and Receiver
- utility functions of Sender and Receiver are related
- Sender assesses decisions (mechanism) robustly: worst-case not expected value



# Differential privacy

## Private mechanisms?

$$\text{Dataset}~x \overset{\text{release}}{\longrightarrow} \text{statistic}~T$$

- $T \sim M(x,\cdot)$, with some mechanism $M$

::: {.fragment}
**Example**
:::

- $T = \frac{1}{n}\sum_{i=1}^{n} x_i + Z$ with $Z \sim \mathcal{N}(0,\sigma^2)$

::: {.fragment}
$$M(x,\cdot) = \mathcal{N}(\overline{x},\sigma^2)$$
:::

## Statistical mechanisms

- Summary statistics $\quad\tau:\mathsf{X}\rightarrow\mathsf{T}$
  - $M(x,\cdot) = \delta_{\tau(x)}(\cdot)$ 

- Noisy summary statistics
  - $M(x,\cdot) = \mathcal{N}(~\cdot \mid  \tau(x), \sigma^2)$
  
- Composition of multiple mechanisms

## Statistical mechanisms

- Result of optimisation
  - MLE: $\quad M(x,\cdot) = \delta_{\tau(x)}(\cdot)$
  - $\tau(x) = \arg\max_{\theta \in \Theta} \log L(x \mid \theta)$
  
- Monte Carlo output
  - MCMC: $\quad M(x,\mathrm{d}t_{1:N}) = \prod_{i=1}^N K(\mathrm{d}t_{i} \mid t_{i-1}, x)$

- Noisy optimisation
  - SGD: $\quad M(x,\mathrm{d}t_N) = \int_{\mathsf{T}_{-n}}\prod_{i=1}^N K(\mathrm{d}t_{i} \mid t_{i-1}, x)$


## DP Intuition

```{r}
library(dplyr)
library(ggplot2)
library(ggthemes)
library(patchwork)
library(tidyr)
library(distributional)
library(ggdensity)


thm <- theme_bw() + # Use a clean theme first
  theme(
    # Remove grid lines
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    # Remove strip background and text
    strip.background = element_blank(),
    strip.text = element_blank(),
    axis.title.x=element_blank(),
    axis.text.x=element_blank(),
    axis.ticks.x=element_blank(),
    axis.title.y=element_blank(),
    axis.text.y=element_blank(),
    axis.ticks.y=element_blank(),
    legend.position = "none",
    text = element_text(size = 20)
  )

mvn1 <- tibble(x = rnorm(100, mean = 2, sd = 1)) |>
        mutate(y = 0.1*(x-2) + rnorm(100, mean = 1, sd = 0.75))

mvn2 <- tibble(x = rnorm(100, mean = -1, sd = 1)) |>
  mutate(y = 0.2*(x) + rnorm(100, mean = 1, sd = 0.25))

mvn <- bind_rows(mvn1,mvn2, .id = "dist")


dist_plot <- ggplot(mvn) + 
  geom_hdr(aes(x,y,fill = dist), method = "mvnorm", probs = c(0.5,0.9,0.99)) +
  scale_x_continuous(limits = c(-4,5)) +
  scale_y_continuous(limits = c(-2,4)) +
  thm + ggtitle(expression(paste("Output: ",  M(x,{}%.%{}))))

point_plot <- ggplot(tibble(x=c(2,-1), y = c(1,1), dist = c("1","2"))) + 
  geom_point(aes(x,y, colour = dist), size = 6) + 
  scale_x_continuous(limits = c(-4,5)) +
  scale_y_continuous(limits = c(-2,4)) +
  thm + ggtitle(expression(paste("Input: ", x))) +
  geom_vline(xintercept = -1, colour = "grey", alpha = 0.5) +
  geom_hline(yintercept = 1, colour = "grey", alpha = 0.5)

overall_plot <- point_plot + dist_plot

overall_plot

```

To limit information leaked about data $x$ by statistic $T$,

control the sensitivity of the mechanism $M(x,\cdot)$ w.r.t. $x$.



## DP Definition

Borrow from Lipschitz continuity [@bailie2024general].

::: {.fragment}
$$d_\text{P}[M(x,\cdot),M(x^\prime,\cdot)] \leq \epsilon~d_\text{X}[x,x^\prime]$$
:::

::: {.fragment}
for all $x,x^\prime \in \mathcal{D}$.
:::

## DP Definition

A mechanism $M$ is $\epsilon$-DP if

::: {.fragment}
$$\sup_{S\in\mathcal{S}}\left\vert\ln\frac{M(x,S)}{M(x^\prime,S)}\right\vert \leq \epsilon$$
:::

::: {.fragment}
for all $x,x^\prime \in \mathcal{D}$, differing by at most one element.
:::

::: {.fragment}

:::{.callout-note}
## Pure Differential Privacy

$$M(x,S) \leq \exp\{\epsilon\} M(x^\prime, S)$$
for all $x \sim x^\prime$ and $S \in \mathcal{S}$.

:::
:::

## Some DP Properties

Focusing on (pure) $\epsilon$-DP.

1. **Composition**: If $M_1$ and $M_2$ are $\epsilon$-DP then

::: {.fragment}

$$M_1 \otimes M_2 ~\text{is}~ 2\epsilon\text{-DP}$$

:::

2. **Post-processing**: If $K$ is independent of the data then
 
::: {.fragment}

$$M_1 K ~\text{is}~ \epsilon\text{-DP}$$

:::

## Some DP Properties

Focusing on $\epsilon$-DP. Examples:

:::{.nonincremental}

1. **Composition**:

If releasing the mean and variance of a dataset are each $\epsilon$-DP, then the joint release is $2\epsilon$-DP.

2. **Post-processing**:

If $M$ constructs a histogram that is $\epsilon$-DP, then any quantile from the histogram will also be $\epsilon$-DP.

:::

## Variants of DP

There are many variants and relaxations of DP.

- Probabilistic DP [@guingona2023comparing]

::: {.fragment}
$$\inf_{x\sim x^\prime}\mathbb{P}_{x}\left[ m(x,T) \leq \exp\{\epsilon\} m(x^\prime, T) \right] \geq 1 - \delta$$
:::

- Approximate DP [@dwork2006approxdp]

::: {.fragment}
$$M(x,S) \leq \exp\{\epsilon\} M(x^\prime, S)  + \delta$$
for all $S \in \mathcal{S}$.
:::

## Variants of DP

There are many variants and relaxations of DP.

- Rényi DP
- Gaussian DP
- Concentrated DP
- Many more!

:::{.fragment}
Related to Lipschitz interpretation 
:::

## Pure DP Semantics

- Hypothesis testing
  - Type I & II error controlled by $\epsilon$ [@wasserman2010statistical]
- Bayesian evidence
  - Prior-to-posterior or posterior-to-posterior ratio controlled [@dwork2006calibrating;@kasiviswanathan2014semantics]
- Game theory 
  - Controls expected utility [@zhang2021more]
  
::: {.fragment}

[Semantics are post hoc!]{style="color:red;"}

:::

## DP and MCMC

::: {.callout-note}

## Private MCMC

Want to privately sample from $\pi(\theta\mid T)$ using MCMC
:::

:::{.fragment}
**Via post-processing**
:::

1. Determine DP guarantee for $$T^\prime = \frac{1}{n}\sum_{i=1}^n T(y_i) + \varepsilon$$
2. Sample from approximate target $\pi(\theta\mid T = T^\prime)$


## DP and MCMC

::: {.callout-note}

## Private MCMC

Want to privately sample from $\pi(\theta\mid T)$ using MCMC
:::

**Via post-processing**

_Or some "exact" alternatives:_

- Sample from noisy-ABC posterior [@fearnhead2012constructing]

- Target posterior of new summary statistic: noisy-ABC + Insufficient Gibbs [@luciano2024insufficient]

::: {.fragment}

[Statistical efficiency loss]{style="color:red;"} and [no extra privacy]{style="color:red;"} from sampling

:::


## DP and MCMC

**Via composition**

::: {.fragment} 

If $M$ is $\pi$-invariant MCMC kernel

:::

1. Establish DP for one sample from $M$
2. Use (advanced) composition property for $N$ samples

- But privacy degrades in $\mathcal{O}(N)$ or $\mathcal{O}(\sqrt{N})$


::: {.fragment}

[Antithetical to monitoring convergence]{style="color:red;"} and prohibits large $N$

:::

::: {.fragment}
_Non-exact alternative:_ Generalised Bayes [@jewson2023differentially]

:::

## Limitations of DP

- DP Semantics are post-hoc explanations
- DP for MCMC is [c o o k e d]{.redacted}
- Deterministic mechanisms are not covered by DP

## Limitations of DP

::: {.fragment}

Our approach is a **semantics first** understanding of data privacy, where

:::

- Privacy definitions are systematically constructed from an **agent-based game**
- Game assumptions have real-world interpretations
- Privacy easier to interpret, communicate and tailor
  
# Persuasive Privacy

## Privacy classes

**Overall aim**: Define useful and defensible privacy definitions.

:::{.fragment}

::: {.callout-note}
## Privacy class

A set of mechanisms that satisfy a given privacy definition.

:::
:::

:::{.fragment}
If $\mathfrak{C}$ is the privacy class generated by "$\text{D}$" then 

$$M \in \mathfrak{C} \Longleftrightarrow M~\text{satisfies}~\text{D}$$

:::


## The players

:::{.callout-tip}
## Sender

:::{.fragment}
Government agency release **small area statistics** on a disease
:::

:::

:::{.callout-warning}
## Receiver

:::{.fragment}
Insurance company **raises premiums** using inferred disease prevalence in a small town
:::

:::

:::{.callout-important}
## Effect on data privacy 

:::{.fragment}
Statistic release $\longrightarrow$ adversarial decisions $\longrightarrow$ privacy effect
:::

:::

## Privacy game {.hidden-title .scrollable}

{{< include diagram.qmd >}}

## A privacy definition

:::{.callout-note}
## Definition: Persuasive Privacy

We say $M$ is $(\mathcal{Q}_x, S, \kappa, \delta)$-PP if

$$\inf_{x\in\mathsf{X}}\inf_{Q\in\mathcal{Q}_x}\mathbb{P}_x\left[S(Q, x) - S(Q_{T}, x) \leq \kappa \right] \geq 1 - \delta,$$

where $\mathbb{P}_x$ is w.r.t. $T \sim M(x,\cdot)$.

:::

- Maximum allowable privacy change: $\kappa$
- Maximum probability of leakage: $\delta\quad$


## Privacy function

Sender's privacy measured with a **privacy function**

$$\rho:\mathcal{D} \times \mathsf{X} \rightarrow \mathbb{R}$$

- Receiver's decision $\quad d_i \in \mathcal{D}$

- Data value $\quad x \in \mathsf{X}$

:::{.fragment}

$\rho$ orders preferences of decisions for a given dataset
:::


- If $d_1$ is preferred to $d_2$ then $\rho(d_1,x) > \rho(d_2,x)$


## Example 1: Privacy function

For some fixed $\kappa>0$,

$$\rho(d,x) = \begin{cases}
  0, & \text{if } x \in d, \vert d \vert < \kappa\\
  1, & \text{otherwise},
\end{cases}$$


:::{.nonincremental}

- $x \in \mathsf{X} = \mathbb{R}$

- $d \in \mathcal{D} = \{[a,b]:a,b\in \mathbb{R}, a\leq b\}$
:::


## Example 2: Privacy function

<br>

$$\rho(d,x) = -\log d(x)$$

:::{.nonincremental}

- $x \in \mathsf{X} = \mathbb{R}$

- $d \in \mathcal{D} = \{\text{probability density functions on } \mathbb{R}\}$
:::

::: {.fragment}

Privacy functions or **scores** chosen to be [fit for purpose]{style="color:red;"}

:::



## Properties of Persuasive Privacy

1. **Composition** $$M_i \in \mathfrak{C} \Rightarrow M_1 \otimes M_2 \in \mathfrak{C}^\prime$$

2. **Receiver post-processing** $$M \in \mathfrak{C} \Rightarrow M \otimes K \in \mathfrak{C}$$

3. **Does not** satisfy (Sender) post-processing $$M \in \mathfrak{C} \Rightarrow M K \in \mathfrak{C}$$

## Returning to DP

Recall the Persuasive Privacy elements $(\mathcal{Q}_x, S, \kappa, \delta)$, and consider

1. Family of alternative hypothesis priors

::: {.fragment}
$$\mathcal{H}_x  = \{ Q \in \mathcal P_2:x^\prime \sim x, Q(\{ x , x^\prime \})= 1 \}$$
:::

2. Log-probability scoring rule 

::: {.fragment}
$$S(P,x) = -\log p(x)$$
:::

## Returning to DP

Alternative hypothesis prior class and log-probability score recover PrDP as instance of Persuasive Privacy

::: {.fragment}
$$\inf_{x\sim x^\prime}\mathbb{P}_{x}\left[ m(x,T) \leq \exp\{\epsilon\} m(x^\prime, T) \right] \geq 1 - \delta$$
::: 

- Pure DP is a special case ($\delta = 0$)
- PrDP implies Approximate DP

## Returning to DP

- Pure, Probabilistic, and approximate DP are notions of privacy relative to change in Receiver's knowledge 
- Can be derived by considering persuasive privacy game
- ADP satisfies (Sender) post-processing property
  - PrDP **does not** (unless pure DP)
  - Typically a criticism of PrDP
- PrDP does have (Receiver) post-processing property
    - A simple fix: control Receiver's decision by including pre-processed statistic
  

## What else is in the paper?

- Probabilistic and pure **DP special cases**
- Rényi and $f$-divergence DP by changing assumptions^[See also: Bayesian Adversarial Privacy [@bell2026bayesian]]
- Privacy developed for **deterministic mechanisms**:
  - empirical mean
  - cell suppression
  
## What is not in the paper?

Current work: [Privacy for MCMC mechanisms]{style="color:red;"}


- Suppose $\tau(x)$ is a sufficient statistic for $\Pi(\;\cdot\mid x)$, and
- $T_{1:N}$ are MCMC samples for $\Pi(\;\cdot\mid x)$, then

:::{.fragment}
$$\tau(x) \quad \overset{\text{information}}{\equiv} \quad (T_{1:\infty}, \Pi)$$

:::

## What is not in the paper?

Current work: [Prove "decomposition" property]{style="color:red;"} for samplers

:::{.fragment}
**Persuasive privacy via decomposition** 
:::

1. Prove privacy for minimally sufficient statistic for $\Pi(\;\cdot\mid x)$
2. Apply decomposition property

:::{.fragment}
Future work: Characterise additional privacy from randomness
:::

## Summary

- Framework for purpose-driven privacy definitions (rigorously justified)
  - All assumptions stated, anatomy used as backbone
- Assessment of existing privacy guarantees with game theory
- New interpretations of post-processing property
  - Easy fix for PrDP post-processing
- Established that privacy guarantees possible for **deterministic algorithms**

## An invitation to (Bayesian) privacy

:::: {.columns}

::: {.column width="65%"}

- $Q \in \mathcal{Q}_x$ **BNP**
- $M(x,\cdot)$ **BayesComp**
- $S(P,x)$ **BayesComp**, **Applied Bayes**
- Post-processing **MV-Martingales**??

::: {.fragment}

**Thank you!**

:::

:::

::: {.column width="35%"}

![](imgs/crossiant.jpg)

:::

::::


# Appendix

# Deterministic example

## Privacy for deterministic mechanisms

- DP can't be defined for deterministic mechanisms
- Persuasive privacy framework flexible enough
- Demonstrate privacy for mean $\bar x$

:::{.fragment}
$$M(x,\cdot) = \delta_{\bar x}(\cdot)$$

:::


## Privacy for deterministic mechanisms

Use class of Gaussian distributions

:::{.fragment}

$$\mathcal{G}_{x}^{r} = \left\{ \mathcal{N}(\mu,\Sigma): \frac{(\bar{x}-\bar{\mu})^2}{\overline{\Sigma}} \leq r_1 , c_\Phi \leq r_2 \left(1- \frac{\sigma_i^2}{\Vert\sigma \Vert_2^2} \right) \right\}$$

for $r_1 > 0$ and $r_2 > 1$. 

:::

- Data posterior after observing $\bar x$ is multivariate normal but degenerate
- Support only on subspace $\{z\in \mathbb{R}^n: \bar z = \bar x\}$

## Privacy for deterministic mechanisms

Use (marginal) Dawid--Sebastiani Score

$$ D_i(Q,x) = \log \sigma^2_i(Q) + \frac{[x_i - \mu_i(Q)]^2}{\sigma^{2}_i(Q)}$$

for each element of data, and consider the worst-case element for privacy.

## Privacy for deterministic mechanisms

:::{.callout-note}
## Proposition 

The average mechanism 
    $M(x,\cdot) = \delta_{\bar{x}}$ satisfies $(\mathcal{I},\mathcal{G}_{x}^{r},r_1+ \log r_2,0)$-PP.
:::

- Possible to have a rigorous guarantee for a deterministic mechanism
- Opens path for privacy guarantees for convergent iterative algorithms
  - e.g. MCMC without privacy perturbations (posterior summary statistics)
  - A "decomposition rule"

# Game-theoretic foundations

## Transparency

**Privacy class**: A set of mechanisms that satisfy a given privacy definition.

:::{.fragment}
If $\mathfrak{C}$ is the privacy class generated by "$\text{D}$" then 

$$M \in \mathfrak{C} \Longleftrightarrow M~\text{satisfies}~\text{D}$$

:::

:::{.fragment}

:::{.callout-note}
## Assumption 1: Transparency

Sender shares the mechanism $M$ and privacy class $\mathfrak{C}$, for which $M \in \mathfrak{C}$, with Receiver. Further, the
definitions of $M$ and $\mathfrak{C}$ do not depend on the data.

:::

:::

## Receiver

:::{.callout-note}
## Assumption 2: Bayesian adversary
Receiver is Bayes rational
:::

1. Holds a prior distribution over the data $Q$
2. Constructs a posterior distribution $Q_T$ from $(M,T)$
3. Has loss function $\ell: \mathcal{D} \times \mathsf{X} \rightarrow \mathbb{R}$

:::{.fragment}
_Sharing $\mathfrak{C}$ does not affect Receiver's data posterior by Assumption 1._
:::



## Receiver's optimal decision

From Assumption 1:

:::{.callout-note}
## Receiver's optimal decision
$$
d^{P} \in \arg\inf_{d \in \mathcal{D}} \mathbb{E}_{z \sim P}[ \ell(d,z)]
$$
under belief $P \in \{Q,Q_T\}$
:::

::: {.fragment}
To model Receiver's decision:
:::

::: {.fragment}
**We need assumptions on $Q$ and $\ell$**
:::

## Receiver model

:::{.callout-note}
## Assumption 3: Adversarial loss function
Receiver has loss function $\ell(d,x) = \rho(d,x)$
:::

- Interpretation 1: Receiver targets privacy

:::{.fragment}
If $d^P_{\ell^\prime}$ is Receiver's optimal decision under $(P,\ell^\prime)$ then
:::

::: {.fragment}
$$\mathbb{E}_{x \sim P}[ \rho(d^{P}_\rho,x)] \leq \mathbb{E}_{x \sim P}[ \rho(d^P_{\ell^\prime},x)]$$
:::


- Interpretation 2: data-averaged **worst-case** for Sender

## Realised privacy outcome

Let $S: \mathcal{P} \times \mathsf{X} \rightarrow \mathbb{R}$ such that 

$$S(P,x) = \rho(d^{P},x)$$

- If $\ell = \rho$, the realised privacy value is a (negatively-orientated) **proper scoring rule**^[@grunwald2004game]
  - A _privacy score_
  
## Privacy scores

- We continue with privacy scores (equivalence)
- Use known proper scoring rules to measure privacy outcome
- Create new proper scoring rules from $\rho$
- For example when $d$ is a PDF/PMF:

:::{.fragment}
$\rho(d,x) = -\log d(x)$ then $S(Q_T,x)=-\log q_T(x)$. 
:::

:::{.fragment}
Without loss of generality, we focus on proper scoring rules. 
:::

<!--
## Detour: Proper scoring rules

Proper scoring rules^[@gneiting2007strictly] provide a convenient (and rich) class of functions to assess privacy!

- Consider space of probability distributions $\mathscr{P}$ on $(\mathsf{X},\mathcal{X})$.

- A scoring rule is a function $S:\mathscr{P} \times \mathsf{X} \rightarrow \mathbb{R}$ 

- Expected score $S(P,Q)$
  - If $P,Q \in \mathscr{P}$ define $S(P,Q) = \mathbb{E}_{z\sim Q} S(P,z)$

## Detour: Proper scoring rules

:::{.callout-note}
## Definition 1: Proper scoring rules^[Negatively-orientated]

A scoring rule $S$ is proper with respect to $\mathscr{P}$ if for all $P,Q \in \mathscr{P}$

$$S(Q,Q) \leq S(P,Q).$$

In addition, it is *strictly* proper if $S(P,P) = S(P,Q) \Leftrightarrow P = Q$.

:::

## Scoring rules and privacy

**Forecasting**

- $S(P,x)$ compares a forecast $P$ to a ground truth $x$

- Propriety encourages truthful reporting of the forecast

**Privacy**

- $S(Q,x)$ compares uncertain adversarial knowledge of the data $Q$ to the true data $x$

- Propriety ensures coherent assessment of the privacy


-->

## Measuring privacy

Relative privacy $\Delta(Q,T,x) = S(Q,x) - S(Q_T,x)$

::: {.fragment}
::: {.callout-note}
## Assumption 4 

For a given $Q$ and $x$, Sender considers a mechanism $M$ private only if

$$\mathbb{P}_x\left[\Delta(Q,T,x) \leq \kappa \right] \geq 1 - \delta,$$

:::
:::

- Maximum acceptable privacy loss $\kappa \geq 0$
- Small probability of failure $0 \leq \delta \ll 1$
- Probability $\mathbb{P}_x$ is w.r.t. $M(x,\cdot)$

## Sender is robust

To be robust, sender should assess 

 - "Realistic" data-priors $Q$
 - All $x \in \mathsf{X}$ (also required by _Assumption 1_)

::: {.fragment}
::: {.callout-note}
## Assumption 5 

Sender considers $M$ private if 

$$\mathbb{P}_x\left[\Delta(Q,T,x) \leq \kappa \right] \geq 1 - \delta$$

holds uniformly for all data-priors $Q \in \mathcal{Q}_x \subset \mathcal{P}(\mathsf{X})$ and datasets $x \in \mathsf{X}$. 

:::
:::

# Persuasive privacy

## A privacy definition

:::{.callout-note}
## Definition: Persuasive Privacy

We say $M$ is $(\mathcal{Q}_x, S, \kappa, \delta)$-PP if

$$\inf_{x\in\mathsf{X}}\inf_{Q\in\mathcal{Q}_x}\mathbb{P}_x\left[\Delta(Q,T, x) \leq \kappa \right] \geq 1 - \delta,$$

where $\mathbb{P}_x$ is w.r.t. $T \sim M(x,\cdot)$.

:::

- Change in privacy: $\Delta(Q,T, x) = S(Q, x) - S(Q_{T}, x)$
- Maximum allowable privacy change: $\kappa$
- Maximum probability of leakage: $\delta\quad$

## Ongoing work

**Short term**

- Decomposition rule (for deterministic mechanisms)
  - Use noise in MCMC as additional privacy

- Nonparametric prior families 

- Independent Commissioner Against Corruption (SA)

**Medium term**
  
- Privacy-utility trade-off

- Computation (SMC, SBI) versus privacy approximation trade-off






## References

::: {#refs}
:::



