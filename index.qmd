...
:::{.fragment}
Insurance company **raises premiums** based on inferred prevalence in a small town.
:::
...
We measure the privacy effect with a **privacy function**

$$\rho:\mathcal{D} \times \mathsf{X} \rightarrow \mathbb{R}$$

- Adversarial decision $\quad d_i \in \mathcal{D}$
...
:::{.callout-note}
## Assumption 1: A Bayesian adversary
The adversary makes Bayesian decisions
:::
...
\node[anchor=south east, xshift=-0.9cm] at (statbox.north) {Statistician};
\node[anchor=south east] at (advbox.north) {Adversary model};

\end{tikzpicture}

```

## Adversary's optimal decision
...
The adversary has loss function $\ell(d,x) = \rho(d,x)$
:::

- Interpretation 1: Adversary targets privacy

:::{.fragment}
If $d^P_{\ell^\prime}$ is the adversary's optimal decision under $(P,\ell^\prime)$ then
:::
...
- Interpretation 2: data-averaged **worst-case** for statistician
...
\node[anchor=south east, xshift=-0.9cm] at (statbox.north) {Statistician};
\node[anchor=south east, xshift=2.35cm] at (advbox.north) {Adversary model + assumptions};
...
The statistician announces a protected set $\mathsf{E} \subset \mathsf{X}$ where the mechanism satisfies
...
- But the adversary learns $x \in \mathsf{E}$
...
```{r dtp5, engine = 'tikz',   fig.align = 'center', cache=TRUE}
...
\node (strans) [pb, below=0.5cm of sstat] {$x \in \mathsf{E}$};
\node (stransd) [desc, left=0pt of strans] {Protected set};
...
- Considers adversary's information gain
- Statistician's relative privacy change (worst-case)
...
- PrDP and ADP assume a minimum strength of the adversary
  - $\mathcal{Q}_x^p = \{Q\in\mathcal{Q}_x:Q(\{x\}) \geq p\}$
- ADP has post-processing invariance property, PrDP **does not**
- From our theory: post-processing by the 
    - adversary **will not** change their posterior distribution
    - statistician **will** change the privacy guarantee
