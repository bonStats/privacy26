...
::: {.fragment}
for all $x,x^\prime \in \mathcal{D}$ some data domain.
:::
...
::: {.fragment}
$$\sup_{S\in\mathcal{S}}\left\vert\ln\frac{M(x,S)}{M(x^\prime,S)}\right\vert \leq \exp(\epsilon)$$
:::
...
We say $M$ is $(\mathcal{Q}_x, S, \kappa, \delta)$-SRP if

$$\inf_{x\in\mathsf{X}}\inf_{Q\in\mathcal{Q}_x}\mathbb{P}_x\left[S(Q_{T}, x) \geq S(Q, x) - \kappa \right] \geq 1 - \delta,$$
